...
try:
    from firestore_sync import (
        init_firestore,
        get_phrases_from_firestore,
        set_phrase_in_firestore,
        seed_firestore as firestore_seed,
    )
except ImportError:
    def init_firestore():
        return None
    def get_phrases_from_firestore(_db):
        return []
    def set_phrase_in_firestore(_db, _text, _safe, _category="phrases", _source="user"):
        return False
    def firestore_seed(_db, _phrases, _category="phrases"):
        return 0
...
class PyxAI:
    ...
    def add_word(self, word: str, safe: bool = True):
        ...
        if safe:
            self._session_safe.add(word)
            self._session_bad.discard(word)
        else:
            self._session_bad.add(word)
            self._session_safe.discard(word)
    ...
    def add_phrase(self, phrase: str, safe: bool = True):
        ...
        if safe:
            self._session_safe.add(phrase)
            self._session_bad.discard(phrase)
        else:
            self._session_bad.add(phrase)
            self._session_safe.discard(phrase)
    ...
    def add_game_idea(self, idea: str, safe: bool = True):
        ...
        if safe:
            self._session_safe.add(idea)
            self._session_bad.discard(idea)
        else:
            self._session_bad.add(idea)
            self._session_safe.discard(idea)
    ...
    def ai_decide(self, text: str, category: str = "phrases") -> Tuple[bool, float]:
        ...
        if safe:
            self._session_safe.add(text)
            self._session_bad.discard(text)
    ...
    def set_label(self, text: str, safe: bool, category: str = "phrases") -> str:
        ...
        Same session: this phrase will always score 0.0 (safe) or 1.0 (bad) until exit.
...
def main():
    ...
    if pyx._db:
        print("Firestore: connected (safe/bad/override will update cloud)")
    else:
        print("Firestore: offline (run with firebase-key.json + pip install firebase-admin to sync)")
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1].strip().lower() == "firestore-check":
        # Show which project the key uses and the exact link to create the database
        try:
            from firestore_sync import _find_key_file
            import json
            path = _find_key_file()
            if not path:
                print("No firebase-key.json or GOOGLE_APPLICATION_CREDENTIALS found.")
                sys.exit(1)
            data = json.loads(Path(path).read_text())
            pid = data.get("project_id", "?")
            print(f"Your key is for project ID: {pid}")
            print(f"Create the (default) Firestore database here:")
            print(f"  https://console.cloud.google.com/datastore/setup?project={pid}")
            print("Steps: 1) Open link  2) Choose 'Firestore in Native mode'  3) Pick location  4) Create  5) Wait 1–2 min  6) Run seed-firestore again")
        except Exception as e:
            print(f"Error: {e}")
        sys.exit(0)
    if len(sys.argv) > 1 and sys.argv[1].strip().lower() == "seed-firestore":
        key_path = sys.argv[2].strip() if len(sys.argv) > 2 else None
        ...
        total = len(TRAINING_GROUNDS_PHRASES)
        print(f"Seeded Firestore with {n} phrases (of {total}). Project: pyx-ai")
        print("  View: https://console.firebase.google.com/project/pyx-ai/firestore/databases/default/data")
        sys.exit(0)
    main()
